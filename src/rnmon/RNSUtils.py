...
def establish_link(
    dest_identity: str, rpc_identity: os.PathLike, dest_type
) -> RNS.Link:
    ...
    RNS.log(
        f"[RNMon] Setting up Destination: {RNS.prettyhexrep(dest_hash)}", RNS.LOG_INFO
    )
    ...
    RNS.log(
        f"[RNMon] Establishing a new link with {RNS.prettyhexrep(dest_hash)}",
        RNS.LOG_DEBUG,
    )
...
def on_link_established(link: RNS.Link, rpc_identity: RNS.Identity) -> None:
    ...
    RNS.log(
        f"[RNMon] KEEPALIVE interval: {link.KEEPALIVE}s, Stale time: {link.stale_time}s",
        RNS.LOG_DEBUG,
    )
...
def ensure_path(dest_hash: bytes) -> None:
    if not RNS.Transport.has_path(dest_hash):
        RNS.log(
            "[RNMon] No path to destination known. Requesting path and waiting for announce to arrive..."
        )
        ...
        while not RNS.Transport.has_path(dest_hash):
            ...
            if time.time() - start > path_request_timeout:
                RNS.log(
                    "[RNMon] Timed out waiting for path announcement", RNS.LOG_ERROR
                )
                raise RuntimeError
...
def set_request_timeout(link: RNS.Link, interval: int) -> int:
    request_timeout = int(
        ...
        + RNS.Resource.RESPONSE_MAX_GRACE_TIME * 1.125
    )
...
def read_rpc_identity(rpc_identity: os.PathLike) -> RNS.Identity:
    ...
    if not mgmt_identity:
        raise FileNotFoundError(
            f"Failed to load identity from {rpc_identity}, check path and permissions."
        )
